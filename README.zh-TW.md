...
當自訂維度數量很多時，可帶入上一頁回傳的 `pageToken` 以繼續分頁查詢。
...
可透過 `pageSize` 控制每頁筆數，並使用 `pageToken` 取得後續頁面。
...
- `resolvePropertyId` 目前只會檢查帳戶摘要與資料串流的第一頁（約各 50 筆）。若組織有大量資源，請改用數字型的 Property ID（例如 `123456789`）以避免查詢失敗。
